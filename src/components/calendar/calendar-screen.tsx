...
import { FERTILE_DISCLAIMER, PREDICTION_DISCLAIMER } from "@/lib/cycle/prediction";
            ...
            <section className="card p-5">
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between gap-3">
                  <dt className="text-[var(--color-muted)]">{CALENDAR.periodRow}</dt>
                  <dd className="text-right">
                    {selectedSpan
                      ? CALENDAR.periodRecorded(formatRange(selectedSpan.start, selectedSpan.end))
                      : CALENDAR.periodNotRecorded}
                  </dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[var(--color-muted)]">{CALENDAR.logRow}</dt>
                  <dd className="text-right">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <Link href={`/app/log/${selectedDate}`}>
                  <Button size="sm" variant="secondary">
                    {selectedLog ? CALENDAR.editLog : CALENDAR.addLog}
                  </Button>
                </Link>
              </div>
            </section>
            ...
            {cycle.data?.prediction ? (
              <section className="card p-5">
                <div className="mb-2 flex items-center justify-between gap-3">
                  <h2 className="text-base font-semibold">{CYCLE.estimatesTitle}</h2>
                  <EstimatedBadge />
                </div>
                <dl className="space-y-2 text-sm">
                  <div className="flex justify-between gap-3">
                    <dt className="text-[var(--color-muted)]">{CYCLE.nextPeriodLabel}</dt>
                    <dd>
                      {formatRange(
                  ...
                  <div className="flex justify-between gap-3">
                    <dt className="text-[var(--color-muted)]">{CYCLE.fertileLabel}</dt>
                    <dd>
                      {formatRange(
        ...
        <PeriodLogForm
          ...
          onRemove={
            selectedSpan
              ? async () => {
                  if (!user) return;
                  await removePeriodSpan(user.uid, selectedSpan.start, selectedSpan.end);
                  await refreshAll();
                  setSheetOpen(false);
                }
              : undefined
